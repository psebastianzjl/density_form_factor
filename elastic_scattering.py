...
tsteps = np.arange(0, 200, 0.5) # time steps
...
for time in tsteps:
    ...
    for i in range(num_traj):
        ...
        if os.path.isfile('./TRAJ{}/Results/'.format(i+1) + filename):
            grid_start, grid_delta, grid_points, density = read_density('./TRAJ{}/Results/'.format(i+1) + filename)
            ...
            if plane == 'xy':
                f0_temp, q_rad, q_phi = transformation(x, y, density)
            if plane == 'yz':
                f0_temp, q_rad, q_phi = transformation(y, z, density, axis=0)
            if plane == 'xz':
                f0_temp, q_rad, q_phi = transformation(x, z, density, axis=1)
            if i == 0:
                f0 = f0_temp
            else:
                f0 += f0_temp
    ...
    plot_polar_signal(q_rad, q_phi, np.absolute(f0) ** 2 / np.amax(np.absolute(f0) ** 2), levels, vmin=0, outname='ued_pyr_{}_absolute_{}_13A'.format(plane, name))
    plot_polar_signal(q_rad, q_phi, np.real(f0) / np.amax(np.absolute(np.real(f0))), levels, vmin=-1.0, outname='ued_pyr_{}_real_{}_13A'.format(plane, name))
    plot_polar_signal(q_rad, q_phi, np.imag(f0) / np.amax(np.absolute(np.imag(f0))), levels, vmin=-1.0, outname='ued_pyr_{}_imag_{}_13A'.format(plane, name))
